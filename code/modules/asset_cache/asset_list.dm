...
#ifdef OPENDREAM
	// OpenDream permits browse_rsc() files per connection, so a mapping cached
	// against an earlier connection can hand out URLs the client is no longer
	// allowed to fetch. Rebuild every time instead of caching.
